...
module.exports = function(app, resource, router) {
  var ioRoutes = {
    get: {},
    post: {},
    delete: {},
  ...
  /*
       Bind middlewares;
       */
  if (resource.middlewares.length) {
    //   console.log('mw:', resource.middlewares)
    //   router.use([(r,s,n) =>{ console.log("1"); n(); },(r,s,n) =>{ console.log("2"); n(); } ]);
    router.use(resource.middlewares);
  }
  ...
  for (var rkey in resource.routes) {
    ...
    for (var path in route) {
      ...
      if (controller[handler_name]) {
        ...
        ioRoutes[normalized.method][normalized.path] = (function(
          _policy,
          _handler
        ) {
